...
LogicalResult matmul_rule_impl(RewriteContext &ctx, Operation &op,
  ...
  // TODO(jevinjiang): remove the constrains for leading axis of rhs needs to be
  // a multiple of 128 in infer/apply layout for vector::contract.
  ...
  CHECK_EQ(padded_rhs_rows, rhs_vregs.dim(0) * layout_rhs.tiling()[0]);
  ...
  // Create vregs filled with all zeros for both LHS, RHS and ACC.
  const VectorType lhs_vreg_type =
      cast<VectorType>(lhs_vregs.begin()->getType());
  FAILUREOR_ASSIGN_OR_RETURN(
      const Attribute lhs_zero_attr,
      getZeroIntOrFloatAttr(lhs_vreg_type.getElementType()));
  auto lhs_zeros_vreg = cast<TypedValue<VectorType>>(
      builder
          .create<arith::ConstantOp>(
              op.getLoc(), DenseElementsAttr::get(lhs_vreg_type, lhs_zero_attr))
          .getResult());
  const auto rhs_vreg_type = cast<VectorType>(rhs_vregs.begin()->getType());
  FAILUREOR_ASSIGN_OR_RETURN(
      const Attribute rhs_zero_attr,
      getZeroIntOrFloatAttr(rhs_vreg_type.getElementType()));
  auto rhs_zeros_vreg = cast<TypedValue<VectorType>>(
      builder
          .create<arith::ConstantOp>(
              op.getLoc(), DenseElementsAttr::get(rhs_vreg_type, rhs_zero_attr))
          .getResult());
  const auto acc_vreg_type = cast<VectorType>(acc_vregs.begin()->getType());
  FAILUREOR_ASSIGN_OR_RETURN(
      const Attribute acc_zero_attr,
      getZeroIntOrFloatAttr(acc_vreg_type.getElementType()));
  auto acc_zeros_vreg = cast<TypedValue<VectorType>>(
      builder
          .create<arith::ConstantOp>(
              op.getLoc(), DenseElementsAttr::get(acc_vreg_type, acc_zero_attr))
          .getResult());

  // Mask out the paddings on both rows and cols of LHS and RHS.
  maskVregs(lhs_vregs, lhs_zeros_vreg, padded_lhs_rows - lhs_shape[0],
            padded_lhs_cols - lhs_shape[1]);
  maskVregs(rhs_vregs, rhs_zeros_vreg, padded_rhs_rows - rhs_shape[0],
            padded_rhs_cols - rhs_shape[1]);

  // At this point, all paddings on vregs are masked out. Now we need to
  // append zero vregs to make LHS's second dim, both RHS's dims and ACC's
  // second dim to be multiples of mxu_size.
  ...
  auto mxu_row_vregs = mxu_size / ctx.target_shape[0];
  ...
  int64_t target_rhs_row_vregs =
      llvm::alignTo(rhs_vregs.dim(0), mxu_row_vregs / layout_rhs.packing());
  ...
  const int64_t rhs_vregs_per_tile =
      mxu_row_vregs / layout_rhs.packing() * mxu_col_vregs;
  ...
  if (transpose_rhs) {
    ...
    CHECK_EQ(nk,
             target_rhs_vregs.dim(0) * layout_rhs.packing() / mxu_row_vregs);
    target_rhs_vregs.Reshape(
